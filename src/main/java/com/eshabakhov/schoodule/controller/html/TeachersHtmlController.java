...
public class TeachersHtmlController {
    ...
    public TeachersHtmlController(final DSLContext datasource) {
    ...
    public String list(
        ...
        if (name != null && !name.isBlank()) {
            condition = condition.and(
                TeachersHtmlController.TEACHER.NAME.likeIgnoreCase(String.format("%%%s%%", name))
            );
        }
        final int limit = 10;
        ...
        model.addAttribute("pageTitle", String.format("%s — учителя", sch.name()));
        model.addAttribute("teachers", teachers.list());
        model.addAttribute("page", offset);
        model.addAttribute("hasNext", teachers.total() > offset * limit);
