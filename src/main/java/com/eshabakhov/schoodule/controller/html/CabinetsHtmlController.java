...
public class CabinetsHtmlController {
    ...
    public String list(
        ...
        model.addAttribute("pageTitle", String.format("%s — кабинеты", sch.name()));
        model.addAttribute("cabinets", cabinets.list());
        model.addAttribute("page", offset);
        model.addAttribute("hasNext", cabinets.total() > offset * limit);
